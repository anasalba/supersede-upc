...
public class Sm4cepParser {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (iri.equalsIgnoreCase("exit")) break;
    ...
    public LinkedList<FunctionParameter> getFunctionParametersList(String functionIRI, String functionTypeIRI) throws ConditionException {
        ...
        while (results.hasNext()) {
            ...
            try {
                ...
                System.out.println("Error with parsing order of a function parameter: " + e);
            }

            // then make a recursive call until it is resolved with no-function operands
            Operand operandInList = this.getOperandForFunctionParameter(operandString);
            FunctionParameter parameterInList = new FunctionParameter(operandInList, order, operandString);
            try {
                parametersList.add(order, parameterInList);
            } catch (IndexOutOfBoundsException ioobx) {
                parametersList.add(parameterInList); // if the element doesn't have element specified or is greater than the current number of elements in the list, it will be added to the end
            }
        }
    ...
    public LinkedList<Condition> getComplexPredicateConditionList(String conditionIRI) throws ConditionException {
        ...
        while (results.hasNext()) {
            ...
            try {
                ...
                System.out.println("Error with parsing condition list order: " + e);
            }
            ...
            try {
                conditionsList.add(order, conditionInList);
            } catch (IndexOutOfBoundsException ioobx) {
                conditionsList.add(conditionInList); // if the element doesn't have element specified or is greater than the current number of elements in the list, it will be added to the end
            }
        }
    ...
    public LinkedList<Operand> getActionOperands(String actionIRI) throws ActionException {
        ...
        while (results.hasNext()) {
            ...
            try {
                ...
                System.out.println("Error with parsing action list order: " + e);
            }

            // this will be recursive call until it is resolved with a simple operand        
            Operand operandInList;
            try {
                operandInList = this.getOperandForFunctionParameter(operandString);
            } catch (ConditionException ex) {
                throw new ActionException("Problem with operands in retrieving action operands!");
            }
            try {
                operandsList.add(order, operandInList);
            } catch (IndexOutOfBoundsException ioobx) {
                operandsList.add(operandInList); // if the element doesn't have element specified or is greater than the current number of elements in the list, it will be added to the end
            }
        }
    ...
    public boolean equalsToSm4cepElement(String retrievedIRI, String sm4cepElement) {
        return retrievedIRI.equalsIgnoreCase("sm4cep:" + sm4cepElement) ||
